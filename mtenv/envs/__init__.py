...
default_kwargs = {
    ...
    "height": 84,
    "width": 84,
    "frame_skip": 4,    # change from 2 to 4
    "frame_stack": 3,
...
for domain_name, task_name, prefix in [
    ...
    ("walker", "walk", "len"),
    ("walker_skill", "walk", "skill"),
]:
    if domain_name != "walker_skill":
        file_ids = list(range(1, 11))
    else:
        # because now have 34 skills, each of them is represented by a .xml
        file_ids = list(range(1, 35))
